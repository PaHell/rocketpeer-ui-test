<script lang="ts">
    ...
    import {center} from '@src/components/NotificationStore'
...
<template>
    ...
    <div class="absolute right-4 p-3 mt-1 text-gray-600 bg-white bg-gray-100 rounded shadow-md messages">
        <ul class="space-y-3">
            {#each $center as message}
            <li class="p-3 border rounded flex">
                <p>{message}</p>
                <div class="mt-1 absolute right-2">
                    <button class="px-2 rounded-sm flex justify-end" on:click={() => handleDismiss(message)}>
